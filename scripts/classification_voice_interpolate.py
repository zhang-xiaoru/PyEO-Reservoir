...
def extendStates(states):
    xp = np.arange(0, states.shape[1] + 1, 1)
    x = np.arange(0.5, states.shape[1] + 0.5, 0.5)
    pad_states = np.roll(states[:, -1:], -1, axis=0)
    extended_states = np.zeros((states.shape[0], len(x)))
    for i, r in enumerate(states):
        f = np.concatenate((pad_states[i], r))
        extended_states[i] = jit_interp(x, xp, f)
    return extended_states
'''
def extendStates(states, num_com_node):
    xp = np.arange(0, states.shape[1] + 1, 1)
    x = np.arange(0.5, states.shape[1] + 0.5, 0.5)
    pad_states = np.roll(states[:, -1:], -1, axis=0)

    def extend_row(r, pad):
        f = np.concatenate((pad, r))
        return jit_interp(x, xp, f)
    result = Parallel(
            ...
            for i in range(0, len(config_list), chunksize):
                ...
                with h5py.File(sim_data_path, "r") as f:
                    for c, config in enumerate(config_chunk):
                        ...
                        extended_hidden_states = extendStates(hidden_states)
                        mean_hidden_states_splt_rm = np.vstack(
                            [
                                np.mean(h[warmup_skip:], axis=0)
                                for h in np.split(
                                    extended_hidden_states, unskiped_split_idx, axis=0
                                )
                            ]
                        )
                        hidden_states_list[c] = mean_hidden_states_splt_rm
