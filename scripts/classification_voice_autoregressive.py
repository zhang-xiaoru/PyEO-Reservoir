            ...
            for i in range(0, len(config_list), chunksize):
                ...
                with h5py.File(sim_data_path, "r") as f:
                    for c, config in enumerate(config_chunk):
                        ...
                        mean_hidden_statesShifted_splt_rm = np.vstack(
                            [
                                np.mean(h[warmup_skip:], axis=0)
                                for h in np.split(
                                    np.roll(hidden_states, 1, axis=0), unskiped_split_idx, axis=0
                                )
                            ]
                        )
                        hidden_states_list[c] = np.hstack(
                            (mean_hidden_states_splt_rm, mean_hidden_statesShifted_splt_rm)
                            )
